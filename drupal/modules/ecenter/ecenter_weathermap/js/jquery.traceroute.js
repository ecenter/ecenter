...
/**
Description of algorithm, for my purposes:

while (hopsIn.length || hopsOut.length) {
  if (hopsIn[0].hop_id == hopsOut[0].hop_id) {
    // DRAW BOTH HOPS
    // Shift array
  
  }
  if (hopsIn[0].hop_id != hopsOut[0].hop_id) {
    // Extend line but don't draw hop for outbound trace
    // if first miss, draw angled line...
...
function TraceRoute(el, options) {
  ...
  traceroute.drawTraceroute(this);
  //$('.' + options.tracerouteClass, el).each(function(i) {
  //});
...
TraceRoute.prototype.drawTraceroute = function(traceroute) {
  var hops = $('.' + this.options.hopWrapperClass, traceroute);
