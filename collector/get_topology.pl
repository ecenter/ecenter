...
sub parse_topo {
	...
	my $description = extract( find( $node, "./nmtb:description", 0), 1);
        my $longitude   = extract( find( $node, "./nmtb:longitude", 0), 1);
        my $latitude    = extract( find( $node, "./nmtb:latitude", 0), 1);
	...
	my $l2_ports =   find( $node, "./nmtl2:port", 0 );
	    ...
	    my $capacity = extract( find( $port2, "./nmtl2:capacity", 0), 1);
	    my $description2 = extract( find( $port2, "./nmtl2:ifDescription", 0), 1);
        ...
        foreach my $port3 ($l3_ports->get_nodelist) {
            my $port2_name    =  extract( find( $port3, "./nmtl3:ifName", 0), 1);
	    my $ip_notted = extract( find( $port3, "./nmtl3:ipAddress", 0), 1);
	    my $netmask = extract( find( $port2, "./nmtl3:netmask", 0), 1);
