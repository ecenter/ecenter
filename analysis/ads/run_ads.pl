...
wrapper script for the ADS deployment

=head1 SYNOPSIS

./run_ads.pl --ports="20500,20501,20502,20503" --logdir=/home/netadmin/ecenter_logs
...
list of ports to run ADS on, comma separated list
Default: 20500
...
kill all previously running ADS instances
Default: not set
...
`/bin/ps auxwww | grep 'app.pl' | grep -v grep | grep -v  nedit  | awk '{print \$2}' | xargs kill -9` if $OPTIONS{clean};
...
foreach my $port (@ports) {
    my $cmd = "plackup  -E production -s Twiggy -a  bin/app.pl -p $port > $OPTIONS{logdir}/ads_$port.log 2>&1 &";
