...
get_remote_md.pl - cache program  with asynchronous calls to the perfSONAR-PS remote LSes and mysql backend
...
./get_remote_md.pl  --key=lhc --procs=20 --password=<db password> --user=<db username>
...
sub get_fromHLS {
   ...
   if( ! (exists $result_disc->{eventType}) || $result_disc->{eventType} =~ m/^error/ ) {
        $logger->error("TID= $$  HLS:: $hls_url got an error when running discovery query", sub {Dumper($result_disc)});
	return;
    }
    if( ! (exists $result_query->{eventType}) ||   $result_query->{eventType} =~ m/^error/ ) {
        $logger->error("TID= $$  HLS:: $hls_url got an error when running  query", sub {Dumper($result_query)});
	return;
    }
    $logger->debug("TID= $$ EventType: $result_disc->{eventType}");
    ...
    if($EVAL_ERROR) {
        $logger->error("TID= $$ This hls $hls_url failed ");
        $hls_obj->is_alive(0);
        return;
    }
	...
	$logger->info("TID= $$  Found for url $param_exist{url} service=" .$service_obj->service);
	...
	$logger->info("TID= $$  +++++++++ Found  something  ", sub{Dumper($look_data_query_id{$id})});
   	# get the keywords
	foreach my $d1_el (@d1_query) {
	    my $data_id =  $d1_el->getAttribute("id");
   	    ...
   	    my %keyword_hash = map { $_ => 1 } 
	        ...
	        $logger->debug("TID= $$ KEYWORD=$keyword_str");
	    ...
	    $logger->info("TID=proc=$$====DATA $id  MD element:::" . $subj_md->toString) if $subj_md;
		...
		my %ip_addr_rs = (   src => '', dst => '');
		    ...
		    if($ips) {
		      ...
		      $logger->info("TID=proc=$$ ======  $try_src=$ip_addr_h{src} ------" . $ips->toString);
		...
		my $host_ip     =  extract( find( $subj_md, "./*[local-name()='interface']/*[local-name()='hostName']", 1), 0 );
	        ...
	        $logger->info("TID=proc=$$ ====== src=$ip_addr_h{src}========== \n");
		...
		foreach my $ip_key (qw/src dst/) {
	        ...
	        if($EVAL_ERROR) {
	            $logger->error("TID= $$  Catched $EVAL_ERROR");
	        }
