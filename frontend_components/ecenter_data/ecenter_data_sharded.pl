...
#
sub get_remote_e2e {
    my ( $md_row,   $params, $type, $metaid, $pm) = @_;
    eval {
	...
	debug "SUBJECT::::::::: $subject";
        ...
        $ma->get_data($request);
        $logger->info("$md_row->{url} MA Data Entries="  . scalar @{$ma->data});
        if($ma->data && @{$ma->data}) {
	    debug "..process data...";
   ...
   #				Code			=> \&get_remote_e2e);	 
	...
	debug " ------  $type md  $metaid  :::  SRC=$md_row->{src_ip} DST= $md_row->{dst_ip} "; 
        ...
        if(  abs($end_time - $params->{$type}{end}) > 1800 ||
   	    ...
   	    my $pid = $pm->start;
            get_remote_e2e($md_row, $params,  $type, $metaid, $pm) unless $pid;
	    debug "CHILD $pid...";
...
sub get_snmp {
    ...
    foreach my $hop_ip (keys %{$hops_ref}) {
        my $data_ref =  _get_snmp_from_db($hop_ip, $date_cond, $params->{snmp});
	...
	unless($data_ref && %{$data_ref->{data}}) {
	    $data_ref =    _get_snmp_from_db($hop_ip, ' ', $params->{snmp});
    ...
    foreach my $hop_ip  (keys %{$hops_ref}) {
       ...
       my $packed = _pack_snmp_data(_get_snmp_from_db($hop_ip, $date_cond,  $params->{snmp}));
       $snmp{$hops_ref->{$hop_ip}} = refactor_result($packed->{data},$params);
    }
    return \%snmp;
}
...
# returns ref to the list of the tables
#
sub _get_shards {
    my ($param) = @_;
    ...
    for ( my $i = $startTime; $i <= $endTime; $i += 86400 ) {
        ...
        my $table = "$param->{data}\_data_$date_fmt";
        $list{$date_fmt} = $table;
    }
    my @tables = ();
    # check if table is there
    foreach my $date_fmt ( sort { $a <=> $b } keys %list ) {
        if ( database->selectrow_array( "select * from   $date_fmt  where 1=0 " )) {
...
sub _get_snmp_from_db{
   ...
   my $data_ref = {};
   my @union_sql = ();
   foreach my $shard (@{_get_shards({%$params, data => 'snmp'})}) {			       
       push @union_sql, qq| (select   distinct  m.metaid,  sd.timestamp as timestamp,  sd.utilization as utilization  
	                                               from 
						            metadata m
                                                	    join $shard sd on(sd.metaid = m.metaid)
						        where 
						            $date_cond
							    m.metaid IN ('$mds')) |;
    }
    if(@union_sql) {
        my $cmd_union = @union_sql>1?join(' UNION ',@union_sql):$union_sql[0];
        debug " SNMP SQL: $cmd_union";
	$data_ref =  database->selectall_hashref( $cmd_union, 'timestamp');
    }
    debug " SNMP SQL: $cmd";
